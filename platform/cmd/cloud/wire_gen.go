...
func wireApp(confServer *conf.Server, confData *conf.Data, auth *conf.Auth, registry *conf.Registry, logger log.Logger) (*kratos.App, func(), error) {
	...
	userUseCase := biz.NewUserUseCase(userRepo, authRepo, node, userEmailVerifyRepo, transaction)
	...
	roomUseCase := biz.NewRoomUseCase(roomRepo, node, userRepo, transaction, roomMemberRepo, redsync)
	roomInstanceRepo := data.NewRoomInstanceRepo(dataData)
	notificationRepo := data.NewNotificationRepo(dataData)
	roomMemberUseCase := biz.NewRoomMemberUseCase(roomMemberRepo, roomRepo, roomInstanceRepo, notificationRepo, node, userRepo, transaction)
	...
	roomInstanceUseCase := biz.NewRoomInstanceUseCase(roomInstanceRepo, node, redsync, gameServerRepo, roomRepo, roomMemberRepo, transaction)
	...
	emulatorGameUseCase := biz.NewEmulatorGameUseCase(emulatorGameRepo, transaction, node, userRepo, auth)
	emulatorServer := service.NewEmulatorService(emulatorUseCase, emulatorGameUseCase)
	grpcServer := server.NewGRPCServer(confServer, userServer, roomServer, roomInstanceServer, notificationServer, roomMemberServer, emulatorServer, logger)
	httpServer := server.NewHTTPServer(confServer, auth, userServer, roomServer, roomInstanceServer, notificationServer, roomMemberServer, emulatorServer, emulatorGameUseCase, logger)
